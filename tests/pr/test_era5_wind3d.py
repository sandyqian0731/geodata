...
def test_wind_interpolation_workflow():
    ...
    Note: The notebook (wind.ipynb) doesn't explicitly call model.prepare() because
    the prepared files already exist from a previous run. In a clean test environment,
    we must call prepare() before estimate().
    """

    # Set up Dask client for parallel processing (matches notebook)
    ...
    try:
        ...
        # Prepare the model (required before estimation in clean environment)
        # Note: In the notebook, this step is skipped because prepared files
        # already exist from a previous run. The model.prepared property checks
        # for existing files and returns True if they exist.
